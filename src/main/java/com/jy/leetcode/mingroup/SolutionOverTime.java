 ...
 */
class SolutionOverTime {
    public static void main(String[] args) {
        SolutionOverTime solution = new SolutionOverTime();
        int[] res = solution.smallestSufficientTeam(new String[]{"cdkpfwkhlfbps", "hnvepiymrmb", "cqrdrqty", "pxivftxovnpf", "uefdllzzmvpaicyl", "idsyvyl"}, createP2());
    }
    ...
    private void minPeople(List<String> reqSkills, List<List<String>> people, int start, List<Integer> res) {
        ...
        for (int i = start; i < people.size(); ++i) {
            ...
            List<String> reqSkillsTemp = new ArrayList<>(reqSkills);
            List<Integer> resTemp = new ArrayList<>(res);

            //使用当前person计算最小团队
            boolean removed = reqSkills.removeAll(people.get(i));
            if (!removed) {
                continue;
            }
            res.add(i);
            ...
            //不使用当前person计算最小团队
            minPeople(reqSkillsTemp, people, start + 1, resTemp);
            if (reqSkills.size() == 0 && reqSkillsTemp.size() == 0) {
                minSize(res, resTemp);
            } else if (reqSkillsTemp.size() == 0) {
                res.clear();
                res.addAll(resTemp);
            }
        }
    ...
    private List<List<String>> createP1() {
        List<String> e1 = new ArrayList<>();
